...
Gnome::Gnome(float m, float f, vec3 col, Particle* particlePTR) : GameObject(1.0f, 0.5f, flag, particlePTR)
{
	mass = m;
	friction = f;
	Colour = col;
...
void Gnome::InitialiseGnome()  //create the 3d object
{
#if 0
	static VertexForGnome GnomeVertices[] =       //Gnome
	{
	...
	//glGenVertexArrays(1, &GnomeVAO);
	//glGenBuffers(1, &GnomeVBO);

	////GnomeVAO = vao;
 //   //GnomeVBO = vbo;
	//glBindVertexArray(GnomeVAO);
	//glBindBuffer(GL_ARRAY_BUFFER, GnomeVBO);
	//glBufferData(GL_ARRAY_BUFFER, sizeof(VertexForGnome) * 36, GnomeVertices, GL_STATIC_DRAW);
	//glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(GnomeVertices[0]), 0);  //layout(location=0) in vec4 fieldCoords;
	//glEnableVertexAttribArray(0);
	//glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(GnomeVertices[0]), (void*)(sizeof(GLfloat)*3));  //layout(location=1) in vec2 fieldTexCoords;
	//glEnableVertexAttribArray(1);
	//glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(GnomeVertices[0]), (void*)(sizeof(GLfloat) * 7));  //layout(location=1) in vec2 fieldTexCoords;
	//glEnableVertexAttribArray(2); 
	//glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(GnomeVertices[0]), (void*)(sizeof(GLfloat) * 10)); //layout(location=1) in vec2 fieldTexCoords;
...
void Gnome::Initialise(unsigned int programId, unsigned int textureID)
{
	...
	unsigned char* data = SOIL_load_image(/*GnomeTextures[colourState]*/grassTexture.c_str(), &width, &height, 0, SOIL_LOAD_RGBA);
...
void Gnome::SetViewMatrix(unsigned int modelViewMatLoc, glm::mat4 modelViewMat)
{
